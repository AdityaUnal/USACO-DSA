    ...
    while(T){
        ...
        if(n%2){
            ...
            for(int i = n - 1;i > 0;i--){
                ...
                cout<<"Post "<<(i - 1)<<" array for odd Places :"<<endl;
                printArray(postOdd[i - 1]);
                cout<<"Post "<<(i - 1)<<" array for even Places :"<<endl;
                printArray(postEven[i - 1]);
            }
            for(int i = 0;i < n;i +=1){
                res = min(res,findmin(preOdd)+findmin(preEven)+findmin(postOdd[i])+findmin(postEven[i]) + 1);
                if(i%2){
                    preOdd[s[i] - 'a'] +=1;
                }
                else{
